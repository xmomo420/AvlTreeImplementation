...
int main() {
  ...
  Livre livre_5(livre_str_5);
	...
	if(!lib_bis.contient(livre_1) || 
			!lib_bis.contient(livre_2) || 
			!lib_bis.contient(livre_3) || 
			!lib_bis.contient(livre_4) || 
			!lib_bis.contient(livre_5)) {
		std::cerr << "ECHEC - II" << std::endl;
		erreur++;
	}
	if(lib_bis.total(livre_1) != 19 || 
		 	lib_bis.total(livre_2) != 17 || 
			lib_bis.total(livre_3) != 12 || 
			lib_bis.total(livre_4) != 27 || 
			lib_bis.total(livre_5) != 13) {
		std::cerr << "ECHEC - III" << std::endl;
		erreur++;
	}
	if(lib_bis.trouver(9784062061612) != livre_1 ||
			lib_bis.trouver(9780886777401) != livre_3 ||
			lib_bis.trouver(9780425191781) != livre_5 ) {
